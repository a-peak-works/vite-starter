  ...
  if (props.href) {
    return (
      <li>
        <a
          href={props.href}
          className="flex items-center justify-between px-4 py-3 tt-md-semi text-primary hover:bg-primary_hover"
        >
          {props.label}
        </a>
      </li>
      ...
      <div className="flex size-full max-w-container flex-1 items-center pr-3 pl-4 md:px-8">
        <div
          className={cx(
            ...
            isFloating &&
              "ring-border-secondary_alt md:rounded-2xl md:bg-primary md:py-3 md:pr-3 md:pl-4 md:shadow-xs md:ring-1",
          )}
        >
                ...
                {items.map((navItem) => (
                  <li key={navItem.label}>
                    {navItem.menu ? (
                      <DialogTrigger
                        defaultOpen={
                          autoOpenMenu && navItem.label === "Resources"
                        }
                      >
